...
function cek_wn($status){
	...
	function get_surat_prosess($id_penduduk){
		return $this->db->query("SELECT * FROM `jenis_surat` WHERE id NOT IN(SELECT surat_id FROM surat_mohon WHERE penduduk_id='$id_penduduk' AND status_surat='review')");
